...
-- QUERY 1
SELECT product_master._description
FROM sales_order
JOIN sales_order_details
ON sales_order._s_order_no = sales_order_details._s_order_no
JOIN client_master
ON client_master._client_no = sales_order._client_no
JOIN product_master
ON product_master._product_no = sales_order_details._product_no
WHERE client_master._name='Ivan';


-- QUERY 2

SELECT product_master._description AS 'Product Name', SUM(sales_order_details._qty_order) AS 'Total quantity to be delivered'
FROM sales_order 
JOIN sales_order_details
ON sales_order._s_order_no = sales_order_details._s_order_no
JOIN product_master
ON product_master._product_no = sales_order_details._product_no
WHERE sales_order._order_status != 'F'
GROUP BY product_master._description;

-- QUERY 3

SELECT product_master._product_no, product_master._description
FROM sales_order 
JOIN sales_order_details
ON sales_order._s_order_no = sales_order_details._s_order_no
JOIN product_master
ON product_master._product_no = sales_order_details._product_no
WHERE sales_order._order_status = 'IP';

-- QUERY 4

SELECT client_master._name
FROM sales_order 
JOIN sales_order_details
ON sales_order._s_order_no = sales_order_details._s_order_no
JOIN product_master
...
SELECT product_master._product_no, sales_order._s_order_no
FROM sales_order
JOIN sales_order_details
ON sales_order._s_order_no = sales_order_details._s_order_no
JOIN product_master
ON product_master._product_no=sales_order_details._product_no
WHERE product_master._description = '1.44 Floppies' AND sales_order_details._qty_order < 5;

-- QUERY 6

SELECT product_master._description, sales_order_details._qty_order
FROM sales_order 
JOIN sales_order_details
ON sales_order._s_order_no = sales_order_details._s_order_no
JOIN product_master
...
JOIN client_master
ON client_master._client_no = sales_order._client_no
WHERE client_master._name IN ('Vandana', 'Ivan'); 
...
JOIN client_master
ON client_master._client_no = sales_order._client_no
WHERE client_master._client_no IN ('0001', '0002');

-- QUERY 8

SELECT sales_order._s_order_no, sales_order._client_no, sales_order._salesman_no
FROM sales_order
GROUP BY sales_order._s_order_no, sales_order._client_no, sales_order._salesman_no
HAVING COUNT(DISTINCT sales_order._salesman_no) > 1;


-- QUERY 9
SELECT DATE_FORMAT(_s_order_date, '%d-%b-%y') AS 'Order Date'
FROM sales_order;


-- QUERY 10
SELECT DATE_ADD(_s_order_date, INTERVAL 15 DAY) AS 'Order Date'
FROM sales_order;
